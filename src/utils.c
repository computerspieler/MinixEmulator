...
int convert_errno()
{
	int linux_minix_errno[] = {
		...
		[EACCES] = MINIX_EACCES,
		[EFAULT] = MINIX_EFAULT,
		[ENOTBLK] = MINIX_ENOTBLK,
		[EBUSY] = MINIX_EBUSY,
		[EEXIST] = MINIX_EEXIST,
		[EXDEV] = MINIX_EXDEV,
		[ENODEV] = MINIX_ENODEV,
		[ENOTDIR] = MINIX_ENOTDIR,
		[EISDIR] = MINIX_EISDIR,
		[EINVAL] = MINIX_EINVAL,
		[ENFILE] = MINIX_ENFILE,
		[EMFILE] = MINIX_EMFILE,
		[ENOTTY] = MINIX_ENOTTY,
		[ETXTBSY] = MINIX_ETXTBSY,
		[EFBIG] = MINIX_EFBIG,
		[ENOSPC] = MINIX_ENOSPC,
		[ESPIPE] = MINIX_ESPIPE,
		[EROFS] = MINIX_EROFS,
		...
		[EDOM] = MINIX_EDOM,
		[ERANGE] = MINIX_ERANGE,
		[EDEADLK] = MINIX_EDEADLK,
		[ENAMETOOLONG] = MINIX_ENAMETOOLONG,
		[ENOLCK] = MINIX_ENOLCK,
		[ENOSYS] = MINIX_ENOSYS,
		[ENOTEMPTY] = MINIX_ENOTEMPTY,
		[EWOULDBLOCK] = MINIX_EWOULDBLOCK,
		[EISCONN] = MINIX_EISCONN,
		[EADDRINUSE] = MINIX_EADDRINUSE,
		[ECONNREFUSED] = MINIX_ECONNREFUSED,
		[ECONNRESET] = MINIX_ECONNRESET,
		[ETIMEDOUT] = MINIX_ETIMEDOUT,
		[ENOTCONN] = MINIX_ENOTCONN,
		[ESHUTDOWN] = MINIX_ESHUTDOWN
	};

	if((size_t) errno >= sizeof(linux_minix_errno)/sizeof(int) || errno < 0)
		return MINIX_EGENERIC;

	return linux_minix_errno[errno];
}